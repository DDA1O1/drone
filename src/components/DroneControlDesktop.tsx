...
import DroneKeyboardControls from './DroneKeyboardControls';

export default function DroneControlDesktop() {
  ...
  const handleConnect = () => {
    console.log('Connect/Disconnect clicked');
  ...
  return (
    <div className="min-h-screen h-screen w-screen bg-black relative overflow-hidden">
      {/* Video feed - full screen */}
      <div className="absolute inset-0 w-full h-full bg-black">
        {isConnected ? (
          isFlying ? (
            <div className="w-16 h-16 border-4 border-blue-500 border-t-transparent rounded-full animate-spin absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
          ) : (
            <div className="text-gray-400 flex flex-col items-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
              </svg>
              <span className="text-lg">Press Takeoff to start flying</span>
            </div>
          )
        ) : (
          <div className="text-gray-400 flex flex-col items-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            <span className="text-lg">Connect to view drone feed</span>
          </div>
        )}
      </div>
